    ...
    for (const k in events) {
      if (events.hasOwnProperty(k)) {
        ...
        if (ev.type === 'VEVENT') {
          ...
          const cleanDesc = (ev.description || '').replace(/St\s+Francis/g, 'St. Francis');
          const cleanDescClean = cleanDesc.replace(/<[^>]+>/g, '').trim();
          const cleanLocation = (ev.location || '').replace(/St\s+Francis/g, 'St. Francis');

          const googleCalLink = `https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(cleanSummary)}&dates=${formatDateForGoogle(ev.start)}/${formatDateForGoogle(ev.end)}&details=${encodeURIComponent(cleanDesc)}&location=${encodeURIComponent(cleanLocation)}`;

          eventList.push({
            summary: cleanSummary,
            description: cleanDesc,
            descriptionClean: cleanDescClean,
        ...
        <div class="card month-card">
          ...
          <ul class="event-list">
            ${monthEvents.map(ev => {
              ...
              return `            <li style="display: flex; flex-direction: column; gap: 0.25rem;">
              <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 0.5rem;">
                <div>
                  <strong>${ev.day} ${ev.monthAbbr}</strong>
                  <span>${escapeHTML(ev.summary)}</span>
                </div>
                <a href="${ev.link}" target="_blank" class="add-to-cal" title="Add to Google Calendar" style="flex-shrink: 0;">+</a>
              </div>
              <div style="margin-top: 0.25rem;">
                <a href="${rsvpUrl}" class="event-rsvp-btn" style="display: inline-flex; align-items: center; gap: 4px; font-size: 0.75rem; color: var(--accent-teal); font-weight: 600; text-decoration: none;">
                  <i data-lucide="send" width="12" height="12"></i> RSVP / Enquire
                </a>
              </div>
            </li>`;
            }).join('\n')}
...
<head>
  ...
  <link rel="canonical" href="https://kougabusinessforum.com/kbevents.html">
  <link rel="icon" href="images/kbf-logo.png" type="image/png">

  <!-- Open Graph / Facebook -->
  <meta property="og:type" content="website">
  <meta property="og:url" content="https://kougabusinessforum.com/kbevents.html">
