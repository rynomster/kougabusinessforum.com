...
async function syncEvents() {
  ...
  if (fs.existsSync(eventsJsonPath)) {
    try {
      ...
      existingEvents = (existing.events || []).map(ev => ({
        ...ev,
        title: ev.title.replace(/St Francis/g, 'St. Francis'),
    ...
    for (let page = 1; page <= MAX_PAGES && hasMorePages; page++) {
      try {
        ...
        for (const itemXml of itemMatches) {
          ...
          const event = {
            title: title.trim().replace(/St Francis/g, 'St. Francis'),
            link,
            guid,
            pubDate,
            ...
            description: cleanDesc.substring(0, 200).replace(/St Francis/g, 'St. Francis'),
            image: localImage
          };
